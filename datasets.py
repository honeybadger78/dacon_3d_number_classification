...
def image_label_dataset(df_path, img_path, div=0.8, grid_shuffle_p=0.8, training=True):
    ...
    if training:
        ...
        train_img, valid_img = img_parser(
            img_path, int(len(all_df) * div), training=training
        )
        return (
            (train_img, valid_img),
...
def train_and_valid_dataload(img_set, label_set, transform, batch_size=16):
    train_loader = custom_dataload(
        img_set[0], label_set[0], batch_size, transform, True
    )
